...
tilt down one YZ meridian (fixed world X) to 45° for a three-stage dendrogram reveal (micro → super →
mega), pull-out on the
same meridian (still 45°), one full CCW dome orbit, tilt to horizontal side view (90°), second full CCW orbit, short hold.
...
# Vertical stems: full thickness at root merges; taper toward leaves (low Z).
DENDRO_STEM_BEVEL_LEAF_FRAC = 0.09
DENDRO_STEM_BEVEL_POWER = 0.48
# Low merges (near leaves): thinner + more transparent so dense stems do not bury points; high merges unchanged.
DENDRO_STEM_ALPHA_LO = 0.2
DENDRO_STEM_ALPHA_HI = 1.0
DENDRO_STEM_ALPHA_CURVE = 0.52
DENDRO_STEM_BEVEL_MUL_LO = 0.32
DENDRO_STEM_BEVEL_MUL_HI = 1.0
DENDRO_STEM_BEVEL_MUL_CURVE = 0.52
CIRCLE_BEVEL = 0.009
# Straight segment between child verticals at a merge (replaces elliptical merge ovals).
MERGE_CHORD_BEVEL = 0.0048
MERGE_CHORD_EMIT_STRENGTH = 1.55
MERGE_CHORD_ALPHA = 0.52
OVAL_COLLAPSE_MINOR = 0.016
...
# Post level-3: YZ meridian tilt → dendrogram → meridian pull-out at 45° → dome yaw 360° → return to top.
# World +Y is scene “north”; side camera uses mostly −Y (``CAM_VIEW_DIR``). More negative = farther back on −Y.
POST_L3_CAM_Y_OFFSET = 2
...
# Timeline: … → L3 top → meridian tilt to 45° → dendrogram → pull back on meridian (still 45°) → dome yaw 360° CCW → top.
F_START = 1
SETTLE_FRAMES = 0
HOLD_L1_TOP = 10
ZOOM_1_2 = 20
...
# Three logical stages (match point-color hierarchy: micro ⊂ super ⊂ mega); equal length in frames.
DENDRO_REVEAL_STAGES = 3
DENDRO_REVEAL_FRAMES_PER_STAGE = 32
# Pull-out zoom runs during the final stage (full tree) at fixed 45° + wider FOV.
DENDRO_TOP_ZOOM_DISTANCE_MUL = 1.45
DENDRO_TOP_ZOOM_LENS_SPAN_MUL = 1.45
# Parallel shift along camera local +Y: final pull-out zoom + first dome orbit; ramps out before tilt to side.
DENDRO_VIEW_PARALLEL_SHIFT = 1.25
DENDRO_TOP_ZOOM_SHIFT_RAMP_IN_FRAMES = 10
DENDRO_DOME_SHIFT_RAMP_OUT_FRAMES = 22
# First CCW orbit at tilt angle after pull-out; then tilt to side (90°) and a second full CCW orbit.
DOME_YAW_ORBIT_FRAMES = 120
TILT_45_TO_SIDE_FRAMES = 26
DOME_SIDE_YAW_ORBIT_FRAMES = 120
HOLD_FINAL_SIDE = 8
# Shift-lens along camera +Y during side view so the look target sits mid–lower frame (not dead center).
SIDE_VIEW_PARALLEL_SHIFT = 2
...
F_DENDRO_REVEAL_END = (
    F_DENDRO_REVEAL_START + DENDRO_REVEAL_STAGES * DENDRO_REVEAL_FRAMES_PER_STAGE - 1
)
F_DENDRO_TOP_ZOOM_START = F_DENDRO_REVEAL_START + (
    DENDRO_REVEAL_STAGES - 1
) * DENDRO_REVEAL_FRAMES_PER_STAGE
...
F_TILT_TO_SIDE_START = F_DOME_ORBIT_END + 1
F_TILT_TO_SIDE_END = F_TILT_TO_SIDE_START + TILT_45_TO_SIDE_FRAMES - 1
F_SIDE_ORBIT_START = F_TILT_TO_SIDE_END + 1
...
def _dendro_shift_scale_for_frame(f):
    """Pull-out zoom: ease in to 1. First dome orbit: hold 1, ease out at end for a clean handoff to side tilt."""
    f = int(f)
...
def internal_merge_reveal_level(nid):
    """1 = within one micro-cluster, 2 = within one super-cluster, 3 = full mega-scale tree."""
    node = nodes[nid]
    mem = node.members
    if len(mem) < 2:
        return DENDRO_REVEAL_STAGES
    n_micro = len({(LEAF_MEGA[i], LEAF_SUPER[i], LEAF_MICRO[i]) for i in mem})
    n_super = len({(LEAF_MEGA[i], LEAF_SUPER[i]) for i in mem})
    if n_micro == 1:
        return 1
    if n_super == 1:
        return 2
...
def connector_up_reveal_level(child_nid):
    """Vertical to parent appears when the parent merge (wider cluster) is revealed."""
    p = parent_of.get(child_nid)
    if p is None:
        return DENDRO_REVEAL_STAGES
    return internal_merge_reveal_level(p)
...
def dendro_stem_bevel_for_height(z_merge, z_base, z_top, bevel_max):
    """Bevel radius: thin at low merges (near leaves), full ``bevel_max`` at the root."""
    span = max(float(z_top) - float(z_base), 1e-9)
    t = (float(z_merge) - float(z_base)) / span
    t = max(0.0, min(1.0, t))
    tw = t ** float(DENDRO_STEM_BEVEL_POWER)
    bmin = float(bevel_max) * float(DENDRO_STEM_BEVEL_LEAF_FRAC)
    return bmin + (float(bevel_max) - bmin) * tw


def dendro_stem_height_t(z_merge, z_base, z_top):
    span = max(float(z_top) - float(z_base), 1e-9)
    t = (float(z_merge) - float(z_base)) / span
    return max(0.0, min(1.0, t))


def dendro_stem_alpha_for_t(t):
    t = max(0.0, min(1.0, float(t)))
    lo, hi = float(DENDRO_STEM_ALPHA_LO), float(DENDRO_STEM_ALPHA_HI)
    g = float(DENDRO_STEM_ALPHA_CURVE)
    return lo + (hi - lo) * (t ** g)


def dendro_stem_bevel_mul_for_t(t):
    t = max(0.0, min(1.0, float(t)))
    lo, hi = float(DENDRO_STEM_BEVEL_MUL_LO), float(DENDRO_STEM_BEVEL_MUL_HI)
    g = float(DENDRO_STEM_BEVEL_MUL_CURVE)
    return lo + (hi - lo) * (t ** g)


_dendro_stem_mat_cache = {}


def dendro_stem_material_for_height_t(t):
    """Shared materials by quantized alpha (few Principled variants)."""
    alpha = dendro_stem_alpha_for_t(t)
    q = round(alpha * 32.0) / 32.0
    if q not in _dendro_stem_mat_cache:
        emit = float(0.52) * (0.2 + 0.8 * max(q, 0.04))
        _dendro_stem_mat_cache[q] = make_principled_material(
            f'DendroStemMat_{int(q * 100 + 0.001):03d}',
            DENDRO_VERTICAL_COLOR,
            roughness=0.12,
            alpha=q,
            emission_strength=emit,
        )
    return _dendro_stem_mat_cache[q]
...
_loc_dome_45 = _look + dome_cam_offset_from_look(
    _TH45, _DOME_PHI_MERIDIAN_MINUS_Y, float(CONE_ORBIT_RADIUS)
)
POSE_DOME_45_ORBIT = (_loc_dome_45, None, float(_lens_orbit), _look)

_lc3 = Vector(TOP_POSE_3[0])
_tt3 = Vector(L3_CAM_TARGET)
_off_top_raw = _lc3 - _tt3
_off_top_fin = _off_top_raw * float(DENDRO_TOP_ZOOM_DISTANCE_MUL) + Vector(
    (0.0, float(POST_L3_CAM_Y_OFFSET), 0.0)
)
_span_top_xy = max(_bx3_x, _bx3_y) * TOP_ZOOM_SPAN_MUL_3 * float(DENDRO_TOP_ZOOM_LENS_SPAN_MUL)
_loc_top_fin = _tt3 + _off_top_fin
_d_top_fin = max(_off_top_fin.length, 1e-6)
_lens_top_fin = perspective_lens_mm_match_span(
    _cam_ref, _span_top_xy, _d_top_fin
)
TOP_POSE_3_POST_FIN = (_loc_top_fin, None, float(_lens_top_fin), _tt3)
...
# -----------------------------------------------------------------------------
# Dendrogram curves + merge chords (horizontal links between child verticals)
# -----------------------------------------------------------------------------
dendro_objects = []
...
for nid, node in nodes.items():
    ...
    _lvl = internal_merge_reveal_level(nid)
    t_sty_l = min(
        dendro_stem_height_t(lz, DENDRO_BASE_Z, Z_HI_FULL),
        dendro_stem_height_t(z, DENDRO_BASE_Z, Z_HI_FULL),
    )
    mat_l = dendro_stem_material_for_height_t(t_sty_l)
    b_l = dendro_stem_bevel_for_height(z, DENDRO_BASE_Z, Z_HI_FULL, _BEVEL_MAX_STEM) * dendro_stem_bevel_mul_for_t(
        t_sty_l
    )
    t_sty_r = min(
        dendro_stem_height_t(rz, DENDRO_BASE_Z, Z_HI_FULL),
        dendro_stem_height_t(z, DENDRO_BASE_Z, Z_HI_FULL),
    )
    mat_r = dendro_stem_material_for_height_t(t_sty_r)
    b_r = dendro_stem_bevel_for_height(z, DENDRO_BASE_Z, Z_HI_FULL, _BEVEL_MAX_STEM) * dendro_stem_bevel_mul_for_t(
        t_sty_r
    )
    ...
    dendro_reveal_specs.append({'obj': conn1, 'reveal_level': _lvl, 'reveal_z': float(z)})
    dendro_reveal_specs.append({'obj': conn2, 'reveal_level': _lvl, 'reveal_z': float(z)})
    ...
    if pid is not None:
        ...
        t_sty_u = min(
            dendro_stem_height_t(z, DENDRO_BASE_Z, Z_HI_FULL),
            dendro_stem_height_t(z_parent, DENDRO_BASE_Z, Z_HI_FULL),
        )
        mat_u = dendro_stem_material_for_height_t(t_sty_u)
        b_up = dendro_stem_bevel_for_height(z, DENDRO_BASE_Z, Z_HI_FULL, _BEVEL_MAX_STEM) * dendro_stem_bevel_mul_for_t(
            t_sty_u
        )
        _lvl_up = connector_up_reveal_level(nid)
        ...
        dendro_reveal_specs.append({'obj': conn_up, 'reveal_level': _lvl_up, 'reveal_z': float(z_parent)})
    ...
    dendro_reveal_specs.append(
        {'obj': chord_obj, 'reveal_level': internal_merge_reveal_level(nid), 'reveal_z': float(z)}
    )
    chord_meta.append({'obj': chord_obj, 'lx': lx, 'ly': ly, 'rx': rx, 'ry': ry, 'z': z})
...
keyframe_merge_chords_follow_vertical_tips(chord_meta, F_START, F_SETTLE_LAST)
...
keyframe_merge_chords_final_hold(chord_meta, F_DENDRO_REVEAL_START)
keyframe_merge_chords_final_hold(chord_meta, F_END)
...
add_caption(
    'Cap_SIDE',
    '45° tilt — dendrogram, pull-out, dome orbit → side view (90°), second full orbit',
    F_CAP_SIDE,
    None,
)
...
# -----------------------------------------------------------------------------
# Dendrogram + merge chords: hidden until three hierarchy-aligned stages (micro → super → mega).
# -----------------------------------------------------------------------------
...
dendro_reveal_specs.sort(key=lambda s: (s['reveal_level'], s['reveal_z'], s['obj'].name))
...
for _spec in dendro_reveal_specs:
    _st = int(_spec['reveal_level'])
    _f_show = F_DENDRO_REVEAL_START + (_st - 1) * DENDRO_REVEAL_FRAMES_PER_STAGE
    _vo = _spec['obj']
...
def _side_view_par_shift_amt(f):
    """Full shift on side orbit + final hold; ease in during 45°→90° meridian so handoff is smooth."""
    f = int(f)
    t0, t1 = int(F_TILT_TO_SIDE_START), int(F_TILT_TO_SIDE_END)
