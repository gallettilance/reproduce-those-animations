...
class HandwriteStyle:
    ...
    frac_scale: float = 0.88
    frac_pad_frac: float = 0.18
    frac_gap_frac: float = 0.70
...
FRAC_SEP = "<<|>>"
FRAC_CLOSE = "<</FRAC>>"
Run = tuple[str, ...]  # hand|hand_b|sub|sup|sym|(limop, op, below, above)|(frac, num, den)
...
def _parse_with_frac(text: str) -> list[Run]:
    """Parse handwriting runs, expanding stacked-fraction markers from TeX."""
    runs: list[Run] = []
    pos = 0
    s = str(text)
    while pos < len(s):
        idx = s.find(FRAC_OPEN, pos)
        if idx < 0:
            runs.extend(_parse_handwrite_runs_plain(s[pos:]))
            break
        if idx > pos:
            runs.extend(_parse_handwrite_runs_plain(s[pos:idx]))
        end = s.find(FRAC_CLOSE, idx + len(FRAC_OPEN))
        sep = s.find(FRAC_SEP, idx + len(FRAC_OPEN))
        if end < 0 or sep < 0 or sep > end:
            runs.extend(_parse_handwrite_runs_plain(s[idx:]))
            break
        num = s[idx + len(FRAC_OPEN):sep]
        den = s[sep + len(FRAC_SEP):end]
        runs.append(("frac", num, den))
        pos = end + len(FRAC_CLOSE)
...
def parse_handwrite_runs(text: str) -> list[Run]:
    ...
    if BOLD_OPEN not in s:
        return _parse_with_frac(s)
    runs: list[Run] = []
    pos = 0
    while pos < len(s):
        idx = s.find(BOLD_OPEN, pos)
        if idx < 0:
            runs.extend(_parse_with_frac(s[pos:]))
            break
        if idx > pos:
            runs.extend(_parse_with_frac(s[pos:idx]))
        end = s.find(BOLD_CLOSE, idx + len(BOLD_OPEN))
        if end < 0:
            runs.extend(_parse_with_frac(s[idx:]))
            break
        inner = s[idx + len(BOLD_OPEN):end]
        runs.extend(_bold_wrap_runs(_parse_with_frac(inner)))
...
def runs_plain(runs: list[Run]) -> str:
    ...
    for run in runs:
        if run[0] == "limop":
            ...
            parts.append(f"{FRAC_OPEN}{run[1]}{FRAC_SEP}{run[2]}{FRAC_CLOSE}")
        else:
...
def frac_size_px(renderer, num: str, den: str, main_fs: float, *, style: HandwriteStyle, bold: bool, fp_hand):
    frac_fs = main_fs * style.frac_scale
    num_runs = parse_handwrite_runs(num)
    den_runs = parse_handwrite_runs(den)
    num_w = mixed_line_width_px(renderer, num_runs, frac_fs, style=style, bold=bold, fp_hand=fp_hand)
    den_w = mixed_line_width_px(renderer, den_runs, frac_fs, style=style, bold=bold, fp_hand=fp_hand)
    num_h = mixed_line_height_px(renderer, num_runs, frac_fs, style=style, bold=bold, fp_hand=fp_hand)
    den_h = mixed_line_height_px(renderer, den_runs, frac_fs, style=style, bold=bold, fp_hand=fp_hand)
    pad = main_fs * style.frac_pad_frac
    gap = main_fs * style.frac_gap_frac
    w_px = max(num_w, den_w) + 2.0 * pad
    h_px = num_h + gap + 1.5 + gap + den_h
...
def mixed_line_width_px(renderer, runs: list[Run], main_fs: float, *, style: HandwriteStyle, bold: bool, fp_hand):
    ...
    for run in runs:
        if run[0] == "limop":
            ...
            w_px, _ = frac_size_px(renderer, run[1], run[2], main_fs, style=style, bold=bold, fp_hand=fp_hand)
            total += w_px
        else:
...
def mixed_line_height_px(renderer, runs: list[Run], main_fs: float, *, style: HandwriteStyle, bold: bool, fp_hand):
    ...
    for run in runs:
        if run[0] == "limop":
            ...
            continue
        if run[0] == "frac":
            _, h_px = frac_size_px(renderer, run[1], run[2], main_fs, style=style, bold=bold, fp_hand=fp_hand)
            frac_ext_px = max(frac_ext_px, h_px - (main_h_px + main_d_px))
            continue
        kind, text = run[0], str(run[1])
        if kind == "sub":
    ...
    return float(main_h_px + main_d_px + sub_ext_px + sup_ext_px + limop_ext_px + frac_ext_px)
    ...
    *,
    ...
    for run in runs:
        if run[0] == "limop":
            ...
            _, main_h_px, main_d_px = renderer.get_text_width_height_descent("x", fp_main, ismath=False)
            frac_fs = main_fs * style.frac_scale
            num_h = mixed_line_height_px(
                renderer, parse_handwrite_runs(run[1]), frac_fs, style=style, bold=bold, fp_hand=fp_hand,
            )
            gap = frac_fs * style.frac_gap_frac
            ascent_px = max(ascent_px, num_h + gap + 0.75 - float(main_d_px))
        elif run[0] == "sup":
    ...
    *,
    ...
    for run in runs:
        ...
        if run[0] == "limop":
            ...
            continue
        if run[0] == "frac":
            num, den = str(run[1]), str(run[2])
            frac_fs = main_fs * style.frac_scale
            num_runs = parse_handwrite_runs(num)
            den_runs = parse_handwrite_runs(den)
            num_w = mixed_line_width_px(renderer, num_runs, frac_fs, style=style, bold=bold, fp_hand=fp_hand)
            den_w = mixed_line_width_px(renderer, den_runs, frac_fs, style=style, bold=bold, fp_hand=fp_hand)
            pad_px = main_fs * style.frac_pad_frac
            gap_px = main_fs * style.frac_gap_frac
            block_w = max(num_w, den_w) + 2.0 * pad_px
            gap_axes = gap_px / ax_h_px
            rule_y = baseline_y
            num_y = rule_y + gap_axes
            den_y = rule_y - gap_axes
            _draw_runs_at(
                target, x_cursor + (block_w - num_w) / (2.0 * ax_w_px), num_y, num_runs, frac_fs,
                style=style, bold=bold, color=color, transform=transform,
                ax_w_px=ax_w_px, ax_h_px=ax_h_px, fp_hand=fp_hand, anchor_va="bottom",
            )
            x0 = x_cursor
            x1 = x_cursor + block_w / ax_w_px
            target.add_line(Line2D(
                [x0, x1], [rule_y, rule_y], transform=transform, color=color,
                linewidth=1.0, clip_on=False, zorder=10,
            ))
            _draw_runs_at(
                target, x_cursor + (block_w - den_w) / (2.0 * ax_w_px), den_y, den_runs, frac_fs,
                style=style, bold=bold, color=color, transform=transform,
                ax_w_px=ax_w_px, ax_h_px=ax_h_px, fp_hand=fp_hand, anchor_va="top",
            )
            x_cursor += block_w / ax_w_px
...
def _tex_emit_frac(num: str, den: str) -> str:
    return f"{FRAC_OPEN}{num}{FRAC_SEP}{den}{FRAC_CLOSE}"
    ...
    while i < n:
        ...
        if ch == "\\":
            if i + 1 < n and s[i + 1] in ",;":
                # LaTeX thin/medium spaces (\, \;), not a punctuation comma.
                out.append(" ")
                i += 2
                continue
            ...
            if cmd in ("sum", "prod"):
                ...
                if above or (below and "=" in below):
                    ...
                    out.append(op)
            elif cmd in ("frac", "dfrac"):
                i = _tex_skip_ws(s, i)
                num, i = _read_braced(s, i)
                i = _tex_skip_ws(s, i)
                den, i = _read_braced(s, i)
                out.append(_tex_emit_frac(_tex_convert(num), _tex_convert(den)))
            elif cmd in ("mathcal",):
                i = _tex_skip_ws(s, i)
                inner, i = _read_braced(s, i)
                glyph = {"L": "ℒ", "N": "N"}.get(inner.strip())
                ...
                if out and not str(out[-1]).endswith((" ", "\n")):
                    out.append(" ")
                out.append("log ")
            elif cmd in ("partial", "nabla", "mid", "cdot", "in", "leftarrow", "infty"):
                repl = {
                    "partial": "∂", "nabla": "∇", "mid": " | ",
...
def _mathtext_wrap(line: str) -> str:
...
def _mathtext_unwrap(line: str) -> str:
    inner = str(line).strip()
...
def _nth_substr_index(text: str, needle: str, occurrence: int = 0) -> int:
    if not needle:
        return -1
    start = 0
    for _ in range(int(occurrence) + 1):
        idx = str(text).find(needle, start)
        if idx < 0:
            return -1
        if _ == int(occurrence):
            return idx
...
def block_anchor_figure_xy(
    ...
    *,
    ...
    text_y_inset_pt: float | None = None,
    y_center_frac: float = 0.62,
) -> tuple[float, float] | None:
    """Figure-coordinate anchor on the first line of a bottom/right block cell."""
    if not anchor:
        return None
    fig.canvas.draw()
    renderer = fig.canvas.get_renderer()
    x0, y0, rw, rh = (float(v) for v in rect)
    fig_w, fig_h = max(float(fig.bbox.width), 1.0), max(float(fig.bbox.height), 1.0)
    ax_w_px, ax_h_px = rw * fig_w, rh * fig_h
    body_fs = float(block.get("block_fs", block_fs))
    ha = "left" if str(block.get("align", "left")) == "left" else "center"
    x_frac = float(text_x_frac if text_x_frac is not None else block.get("text_x_frac", 0.07 if ha == "left" else 0.5))
    x_px = x_frac * ax_w_px
    if block.get("text_x_shift_pt") is not None:
        x_px += block_pt_to_px(fig, block, float(block["text_x_shift_pt"]))
    y_px = block_pt_to_px(
        fig, block,
        float(block.get("top_pad_pt", style.top_pad_pt))
        + float(text_y_inset_pt if text_y_inset_pt is not None else block.get("text_y_inset_pt", 0.0)),
    )
    if block.get("text_y_shift_pt") is not None:
        y_px += block_pt_to_px(fig, block, float(block["text_y_shift_pt"]))

    mlines = block_mathtext_lines(block)
    if mlines:
        mline = mlines[0]
        m_fs = float(block.get("mathtext_fs", body_fs))
        usetex = block_mathtext_usetex(block)
        inner = _mathtext_unwrap(mline)
        idx = _nth_substr_index(inner, anchor, occurrence)
        if idx < 0:
            return None
        prefix_inner = inner[:idx]
        anchor_inner = anchor
        prefix_tex = _mathtext_wrap(prefix_inner)
        anchor_tex = _mathtext_wrap(anchor_inner)
        try:
            prefix_w = mathtext_line_width_px(renderer, prefix_tex, m_fs, usetex=usetex) if prefix_tex else 0.0
            anchor_w = mathtext_line_width_px(renderer, anchor_tex, m_fs, usetex=usetex)
            line_h = mathtext_line_height_px(renderer, _mathtext_wrap(inner), m_fs, usetex=usetex)
        except ValueError:
            return None
        x_fig = x0 + (x_px + prefix_w + 0.42 * anchor_w) / fig_w
        y_fig = y0 + rh - (y_px + float(y_center_frac) * line_h) / fig_h
        return x_fig, y_fig

    raw_lines = [ln for ln in str(block.get("text", "")).split("\n") if ln.strip()]
    disp_lines = block_display_lines(block, style=style)
    disp = disp_lines[0] if disp_lines else (raw_lines[0] if raw_lines else "")
    raw = raw_lines[0] if raw_lines else disp
    idx = _nth_substr_index(raw, anchor, occurrence)
    if idx < 0:
        idx = _nth_substr_index(disp, anchor, occurrence)
    if idx < 0:
        return None
    disp_idx = _nth_substr_index(disp, anchor, occurrence)
    if disp_idx < 0:
        disp_idx = min(idx, max(len(disp) - 1, 0))
    prefix = disp[:disp_idx]
    bold = bool(block.get("bold_lhs", False))
    fp = hand_font(body_fs, bold=bold) if style.enabled else None
    prefix_w = mixed_line_width_px(renderer, parse_handwrite_runs(prefix), body_fs, style=style, bold=bold, fp_hand=fp)
    anchor_w = mixed_line_width_px(renderer, parse_handwrite_runs(anchor), body_fs, style=style, bold=bold, fp_hand=fp)
    x_fig = x0 + (x_px + prefix_w + 0.42 * anchor_w) / fig_w
    line_h = mixed_line_height_px(renderer, parse_handwrite_runs("P"), body_fs, style=style, bold=bold, fp_hand=fp)
    y_fig = y0 + rh - (y_px + float(y_center_frac) * line_h) / fig_h
    return x_fig, y_fig
...
def draw_block_cell(
    ...
    *,
    ...
    while i < len(lines):
        ...
        if h_px <= 0 and bool(block.get("reserve_hidden_lines")):
