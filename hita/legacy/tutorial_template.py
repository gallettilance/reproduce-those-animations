...
class TutorialLayout:
    ...
    bottom_content_lift_frac: float = 0.0
...
class TutorialScene:
    ...
    bottom_arrows: list[dict] | None = None
...
class TutorialComposer:
    def __init__(
        ...
        *,
        ...
        self._dynamic_overlay_cache: dict[tuple, Image.Image] = {}
        self._plot_right_overlay_cache: dict[tuple, Image.Image] = {}
        self._bottom_corner_overlay_cache: dict[tuple, Image.Image] = {}
    ...
    def clear_rails_cache(self) -> None:
        ...
        self._dynamic_overlay_cache.clear()
        self._plot_right_overlay_cache.clear()
        self._bottom_corner_overlay_cache.clear()
    ...
    def layout_rects(self, fig=None) -> dict:
        ...
        bottom_content_y0 += float(L.bottom_content_lift_frac)
    ...
    def _draw_bottom_link_arrows(self, fig, arrows, bottom_rects, *, blocks=None, style=None, ty=None):
        """Optional figure-coordinate arrows between bottom formula columns."""
        from matplotlib.patches import FancyArrowPatch

        if not arrows or not bottom_rects:
            return
        blocks = list(blocks or [])
        for spec in arrows:
            kind = str(spec.get("kind", ""))
            if kind != "ch5_lik_to_bayes" or len(bottom_rects) < 2:
                continue
            bx, by, bw, bh = (float(v) for v in bottom_rects[0])
            lx, ly, lw, lh = (float(v) for v in bottom_rects[1])
            start = end = None
            if blocks and style is not None and ty is not None and len(blocks) >= 2:
                lik_block, bayes_block = blocks[1], blocks[0]
                start_anchor = str(spec.get("start_anchor", "P"))
                end_anchor = str(spec.get("end_anchor", "P(Data"))
                start = hw.block_anchor_figure_xy(
                    fig, lik_block, bottom_rects[1],
                    anchor=start_anchor,
                    occurrence=int(spec.get("start_occurrence", 0)),
                    style=style,
                    block_fs=float(ty.bottom_block_fs),
                    text_x_frac=float(lik_block.get("text_x_frac", 0.04)),
                    text_y_inset_pt=float(lik_block.get("text_y_inset_pt", 2.0)),
                    y_center_frac=float(spec.get("start_y_center_frac", 0.55)),
                )
                end = hw.block_anchor_figure_xy(
                    fig, bayes_block, bottom_rects[0],
                    anchor=end_anchor,
                    occurrence=int(spec.get("end_occurrence", 0)),
                    style=style,
                    block_fs=float(bayes_block.get("block_fs", ty.bottom_block_fs)),
                    text_x_frac=float(bayes_block.get("text_x_frac", 0.06)),
                    text_y_inset_pt=float(bayes_block.get("text_y_inset_pt", 2.0)),
                    y_center_frac=float(spec.get("end_y_center_frac", 0.34)),
                )
            if start is None or end is None:
                sx = float(spec.get("start_x_frac", 0.04))
                sy = float(spec.get("start_y_frac", 0.70))
                ex = float(spec.get("end_x_frac", 0.84))
                ey = float(spec.get("end_y_frac", 0.48))
                start = (lx + lw * sx, ly + lh * sy)
                end = (bx + bw * ex, by + bh * ey)
            color = str(spec.get("color", "#555555"))
            arr = FancyArrowPatch(
                start, end, transform=fig.transFigure,
                arrowstyle="-|>", mutation_scale=14.0, linewidth=1.35,
                color=color, shrinkA=1.0, shrinkB=1.0, zorder=120,
            )
    ...
    def _bottom_ch5_bayes_3col_rects(self, blocks, bottom_rect):
        """ch5_01: new belief | likelihood | probability — one row, three columns."""
        x0, y0, total_w, h = (float(v) for v in bottom_rect)
        gap = float(self.layout.bottom_col_gap_frac)
        bayes_w = max(total_w * 0.30, 0.18)
        lik_w = max(total_w * 0.28, 0.16)
        prob_w = max(total_w - bayes_w - lik_w - 2.0 * gap, 0.22)
        lik_x = x0 + bayes_w + gap
        prob_x = lik_x + lik_w + gap
        return [
            (x0, y0, bayes_w, h),
            (lik_x, y0, lik_w, h),
            (prob_x, y0, prob_w, h),
    ...
    def _bottom_block_rects(self, fig, blocks, bottom_rect):
        ...
        if (
            len(blocks) == 3
            and blocks[0].get("formula_slot") == "bayes_col"
            and blocks[1].get("formula_slot") == "lik"
            and blocks[2].get("formula_slot") == "log"
        ):
            return self._bottom_ch5_bayes_3col_rects(blocks, bottom_rect)
        if (
            len(blocks) == 3
    ...
    def _draw_bottom_blocks_overlay(
        ...
        *,
        ...
        if isinstance(prog, dict) and "bottom" in prog and not any(
            isinstance(k, int) for k in prog.keys()
        ):
            per = prog.get("bottom", {})
        else:
            per = prog if isinstance(prog, dict) else {}
    ...
    def _dynamic_overlay_cache_key(
        self,
        scene: TutorialScene,
        cache_key: str,
        prog: dict,
        *,
        ...
        plot_id = id(scene.plot) if scene.plot is not None else None
        key: list = [cache_key, plot_id, self._freeze_line_progress(prog.get("right", {}))]
        if scene.math_right_blocks:
            key.append(tuple(str(b.get("text", "")) for b in scene.math_right_blocks))
        if include_bottom:
            key.append(self._freeze_line_progress(prog.get("bottom", {})))
            if scene.math_bottom_blocks:
                key.append(tuple(str(b.get("text", "")) for b in scene.math_bottom_blocks))
        key.append(self._freeze_line_progress(prog.get("corner", {})))
        return tuple(key)

    def _plot_right_overlay_cache_key(
        self, scene: TutorialScene, cache_key: str, prog: dict,
    ) -> tuple:
        plot_id = id(scene.plot) if scene.plot is not None else None
        texts = tuple(str(b.get("text", "")) for b in (scene.math_right_blocks or []))
        return (
            cache_key,
            plot_id,
            self._freeze_line_progress(prog.get("right", {})),
            texts,
        )

    def _bottom_corner_overlay_cache_key(
        self, scene: TutorialScene, cache_key: str, prog: dict,
    ) -> tuple:
        bottom_texts = tuple(str(b.get("text", "")) for b in (scene.math_bottom_blocks or []))
        corner_texts = tuple(str(b.get("text", "")) for b in (scene.math_corner_blocks or []))
        return (
            cache_key,
            self._freeze_line_progress(prog.get("bottom", {})),
            bottom_texts,
            self._freeze_line_progress(prog.get("corner", {})),
            corner_texts,
        )

    def _cached_plot_right_overlay(
        self,
        scene: TutorialScene,
        *,
        cache_key: str,
        ...
        key = self._plot_right_overlay_cache_key(scene, cache_key, prog)
        cached = self._plot_right_overlay_cache.get(key)
        if cached is not None:
            return cached
        overlay = self._render_plot_and_right_overlay(
            scene,
            plot_rect=plot_rect,
            ...
            title_u=1.0,
            panel_u=panel_u,
            right_rects_key=right_rects_key,
        )
        self._plot_right_overlay_cache[key] = overlay
        return overlay

    def _render_bottom_corner_overlay(
        self,
        scene: TutorialScene,
        *,
        prog: dict,
        ...
        corner_rects_key: str | None,
    ) -> Image.Image:
        """Bottom formulas + corner notation on a transparent layer."""
        T = self.theme
        ...
        if scene.math_corner_blocks and float(panel_u) > 1e-4:
            corner_rects = self._corner_block_rects_cache.get(corner_rects_key or "corner")
            if corner_rects is None:
                ...
                if corner_rects_key:
                    self._corner_block_rects_cache[corner_rects_key] = corner_rects
            per = prog.get("corner", {})
    ...
    def _cached_bottom_corner_overlay(
        self,
        scene: TutorialScene,
        *,
        cache_key: str,
        ...
        key = self._bottom_corner_overlay_cache_key(scene, cache_key, prog)
        cached = self._bottom_corner_overlay_cache.get(key)
        if cached is not None:
            return cached
        overlay = self._render_bottom_corner_overlay(
            scene,
            prog=prog,
            panel_u=panel_u,
            bottom_rects_key=bottom_rects_key,
            corner_rects_key=corner_rects_key,
        )
        self._bottom_corner_overlay_cache[key] = overlay
        return overlay
    ...
    def _render_plot_right_bottom_overlay(
        ...
        *,
        ...
        if scene.math_corner_blocks and float(panel_u) > 1e-4:
            corner_rects = self._corner_block_rects_cache.get(
                f"{bottom_rects_key or 'corner'}:corner"
            )
            if corner_rects is None:
                corner_rects = self._right_block_rects(
                    fig, scene.math_corner_blocks, rects["math_corner_content"],
                ...
                self._corner_block_rects_cache[
                    f"{bottom_rects_key or 'corner'}:corner"
                ] = corner_rects
            per = prog.get("corner", {})
            for bi, (block, rect) in enumerate(zip(scene.math_corner_blocks, corner_rects)):
    ...
    def _compose_frame_cached_rails(
        ...
        *,
        ...
        overlay_key = self._dynamic_overlay_cache_key(
            scene, cache_key, prog, include_bottom=False,
        )
        dynamic = self._dynamic_overlay_cache.get(overlay_key)
        if dynamic is None:
            dynamic = self._cached_plot_right_overlay(
                scene,
                cache_key=cache_key,
            ...
            self._dynamic_overlay_cache[overlay_key] = dynamic
    ...
    def _compose_frame_cached_shell_rails(
        ...
        *,
        ...
        prog = {
            ...
            "corner": progress_override.get("corner", {}),
        }
        plot_rect = self.layout_rects()["plot"]
        pr = self._cached_plot_right_overlay(
            scene,
            cache_key=shell_key,
            plot_rect=plot_rect,
            plot_alpha=plot_alpha,
        ...
        bc = self._cached_bottom_corner_overlay(
            ...
            corner_rects_key=f"{shell_key}:corner",
        )
        base = static.convert("RGBA")
        out = Image.alpha_composite(base, pr.convert("RGBA"))
        out = Image.alpha_composite(out, bc.convert("RGBA"))
    ...
    def compose_frame(
        ...
        *,
        ...
        if (
            ...
            and po_keys <= {"bottom", "right", "corner"}
        ...
        if scene.math_bottom_blocks and float(panel_u) > 1e-4:
            ...
            self._draw_bottom_link_arrows(
                fig, scene.bottom_arrows, bottom_rects,
                blocks=scene.math_bottom_blocks,
                style=style,
                ty=ty,
            )
